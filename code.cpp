...
#define MQTT_NAME "Our Adafruit name"
#define MQTT_PASS "*******************"
WiFiUDP ntpUDP;
...
int OPEN_ANGLE = 0; // The opening angle of the servo motor
arm
int hh, mm, ss;
int pos;
int pos1;
...
boolean s2_occupied = false;boolean s3_occupied = false;
WiFiClient client;
...
delay(3000);
myservo.write(CLOSE_ANGLE);//if high then count and send
//decrement count
}
...
Serial.println("Occupied2 ");EntryTimeSlot2 = h +" :" + m;
...
{
//Print the new value to the serial monitor
Serial.println((char*) EntryGate.lastread);if (!strcmp((char*) ExitGate.lastread, "ON"))
{
myservo.write(OPEN_ANGLE);
delay(3000);
